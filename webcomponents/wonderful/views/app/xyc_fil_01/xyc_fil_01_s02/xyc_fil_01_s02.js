define(["API", "APIS", 'AppLang', 'views/app/xyc_fil_01/requisition.js', 
'array', 'Directives', 'ReqTestData', 'ionic-popup',
'circulationCardService', 'commonFactory', 'commonService', 
'userInfoService', 'scanTypeService', 'numericalAnalysisService'
, "views/app/xcommon/js/xappLang.js"
    , "views/app/xcommon/js/xcommonService.js"
], function() {
    return ['$scope', '$state', '$stateParams', '$timeout', '$filter', 'AppLang', 'APIService', 'APIBridge',
        ...
        'xyc_fil_01_requisition', 'ReqTestData', 'circulationCardService', 
        'commonFactory', 'commonService', 'userInfoService', 
        'scanTypeService', 'numericalAnalysisService', 'xAppLang', 'xcommonService',
        function($scope, $state, $stateParams, $timeout, $filter, AppLang, APIService, APIBridge,
            $ionicLoading, $ionicPopup, $ionicModal, $ionicListDelegate, IonicPopupService, IonicClosePopupService,
            xyc_fil_01_requisition, ReqTestData, circulationCardService, commonFactory, 
            commonService, userInfoService, scanTypeService, numericalAnalysisService
            , xAppLang, xcommonService) {
            ...
            //將參數傳入xyc_fil_01_requisition
            xyc_fil_01_requisition.setParams(angular.copy($scope.page_params));
            ...
            $scope.scaninfo = {
                in_the_scan: false,
                directly_added: true,
                scanning: xyc_fil_01_requisition.scanning || "",
                search: "",
                 //唛头 simple：2017-00010001
                shipping_mark: "",
                //唛头号 simple：2017-0001
                shipping_mark_no: "",
                focus_me: false,
                focus_sm: true,
                smark_scanning: "",
            ...
            xyc_fil_01_requisition.clearScanning();
  ...
  //检查是否是唛头，检查是否已插入
            $scope.checkSMark = function (scanning) {
                var flag = false;
                var checkData = scanning.toString().trim();
                if (checkData.length > 4) {
                    var shipping_mark_no = checkData.substr(0, checkData.length - 4);
                    var item_doc_qty = xyc_fil_01_requisition.getSourceDocDetail();
                    var index = item_doc_qty.findIndex(function (t) {
                        return commonService.isEquality(t.shipping_mark_no, shipping_mark_no);
                    });
                    if (index > -1) {
                        flag = true;
                    }
            ...
            $scope.setsMark = function (scanning) {
                if ($scope.checkSMark(scanning)) {
                    //已扫描
                    var index = $scope.scanning_detail.findIndex(function (t) {
                        return commonService.isEquality(t.shipping_mark, scanning);
                    });
                    if (index == -1) {
                        $scope.scaninfo.shipping_mark = scanning;
                        var shipping_mark_no = scanning.toString().trim().substr(0, scanning.length - 4);
                        $scope.scaninfo.shipping_mark_no = shipping_mark_no;
                        $scope.setFocusSmark(false);
                        $scope.setFocusMe(true);
                    } else {
                        //已扫描
                        userInfoService.getVoice($scope.xlangs.shipping_mark_error3, function () {
                            $scope.setFocusMe(false);
                            $scope.setFocusSmark(true);
                        });
                    }
                } else {
                    //错误的唛头信息
                    userInfoService.getVoice($scope.xlangs.shipping_mark_error2, function () {
                        $scope.setFocusMe(false);
                        $scope.setFocusSmark(true);
                    });
            ...
            $scope.insertGood = function(temp) {
                ...
                if ($scope.l_data.show_s07_page || $scope.page_params.in_out_no !== "-1") {
                    $scope.bcmeCreate(xyc_fil_01_requisition.parameter);
                }
                return flag;
            };
            ...
            $scope.setFocusSmark= function (flag) {
                if ($scope.scaninfo.focus_sm == flag) {
                    $scope.scaninfo.focus_sm = !flag;
                    $timeout(function () {
                        $scope.scaninfo.focus_sm = flag;
                    }, 0);
                } else {
            ...
            $scope.addGoods = function (obj) {
                if (commonService.isNull($scope.scaninfo.shipping_mark)) {
                    $scope.setFocusMe(false);
                    $scope.setFocusSmark(true);
            ...
            $scope.editGoods = function(index) {
                ...
                $scope.getShowInfo(temp);
                $state.go("xyc_fil_01_s02.xyc_fil_01_s03");
            };
            ...
            var showUpcoming = function(upcoming) {
                ...
                $ionicModal.fromTemplateUrl('views/app/common/html/upcomingModal.html', {
                    ...
                    var bcmeBcaeBcafDelete = function(info_id) {
                        ...
                        APIBridge.callAPI('xyc_bcme_ae_af_delete', [l_data]).then(function (result) {
                            $ionicLoading.hide();
            ...
            $scope.editMultiUnit = function(type, index) {
                        ...
                        switch (type) {
                            ...
                            default:
                                ...
                                if ($scope.page_params.in_out_no == "-1") {
                                    ...
                                    maxqty = (Number($scope.showInfo.used_barcode_inventory_qty) > Number($scope.showInfo.max_allow_doc_qty)) ? $scope.showInfo.max_allow_doc_qty : $scope.showInfo.used_barcode_inventory_qty;
             ...
             //                   return false;
             //               }
                             if (Number($scope.multUnit.all_qty) > Number($scope.multUnit.used_barcode_inventory_qty )) {
                                userInfoService.getVoice($scope.langs.barcode + $scope.langs.stock + $scope.langs.qty + $scope.langs.insufficient + "！", function () {
                                    $scope.setFocusMe(true);
                                    $scope.inTheScan(false);
                                });
                                $scope.multUnit.all_qty = $scope.multUnit.used_barcode_inventory_qty;
                                return false;
                            }
                    ...
                    $scope.setMultiUnitQty = function() {
                        ...
                        for (var k = 0; k < $scope.scanning_detail.length; k++) {
                            ...
                            if ( !( commonService.isEquality(item.shipping_mark_no, $scope.showInfo.shipping_mark_no) &&
                                    commonService.isEquality(item.barcode_no, $scope.multUnit.barcode_no) &&
                        ...
                        for (var j = 0; j < sourceDocDetail.length; j++) {
                            if (!(commonService.isEquality(sourceDocDetail[j].shipping_mark_no, $scope.showInfo.shipping_mark_no)) ||
                                (sourceDocDetail[j].allow_doc_qty === 0 ||
                            ...
                            if ($scope.scanning_detail.length > 0) {
                                ...
                                angular.forEach($scope.scanning_detail, function(value) {
                                    if ((!commonService.isEquality(value.barcode_no, $scope.multUnit.barcode_no) ||
                                            ...
                                            !commonService.isEquality(value.lot_no, $scope.multUnit.lot_no) ||
                                            !commonService.isEquality(value.inventory_management_features, $scope.multUnit.inventory_management_features)) &&
                                        commonService.isEquality(value.shipping_mark_no, array[i].shipping_mark_no) &&
            ...
            $scope.getShowInfo = function(obj) {
                if (angular.isObject(obj)) {
                    $scope.showGood.shipping_mark = obj.shipping_mark;
                    $scope.showGood.shipping_mark_no = obj.shipping_mark_no;
                ...
                var shipping_mark = $scope.showGood.shipping_mark;
                var shipping_mark_no = $scope.showGood.shipping_mark_no;
                ...
                var index = $scope.scanning_detail.findIndex(function (item) {
                    return commonService.isEquality($scope.showGood.barcode_no, item.barcode_no) &&
                        commonService.isEquality($scope.showGood.shipping_mark_no, item.shipping_mark_no) &&
                ...
                for (var i = 0; i < $scope.scanning_detail.length; i++) {
                    ...
                    if (commonService.isEquality(shipping_mark_no, item.shipping_mark_no) &&
                        commonService.isEquality(item.item_no, showInfo.item_no) &&
                        ...
                        if (item.packing_barcode != "N") {
                            ...
                            var index = elementIndex.findIndex(function(element) {
                                return commonService.isEquality(element.barcode_no, item.barcode_no) &&
                                    commonService.isEquality(shipping_mark_no, item.shipping_mark_no) &&
                    ...
                    if (commonService.isEquality(item.barcode_no, showInfo.barcode_no) &&
                        commonService.isEquality(item.warehouse_no, showInfo.warehouse_no) &&
                        commonService.isEquality(item.storage_spaces_no, showInfo.storage_spaces_no) &&
                        commonService.isEquality(item.lot_no, showInfo.lot_no) &&
                        !commonService.isEquality(item.shipping_mark, shipping_mark)
                    ) {
                        used_barcode_inventory_qty = numericalAnalysisService.accAdd(used_barcode_inventory_qty, item.qty);
                    }
                }
                ...
                if ($scope.l_data.has_source) {
                    ...
                    for (var j = 0, len = sourceDocDetail.length; j < len; j++) {
                        ...
                        if (commonService.isEquality(shipping_mark_no, sourceDoc.shipping_mark_no) &&
                            commonService.isEquality(sourceDoc.item_no, showInfo.item_no) &&
                ...
                if (!commonService.isNull(showInfo.barcode_type) || $scope.l_data.show_s07_page) {
                        showInfo.used_barcode_inventory_qty = numericalAnalysisService.accSub(
                        showInfo.barcode_inventory_qty, used_barcode_inventory_qty);
            ...
            $scope.goTodolistNotice = function() {
                ...
                if (commonService.Platform == "Chrome") {
                    xyc_fil_01_requisition.init();
                    $state.go(page);
                    return;
                }
                $ionicLoading.show();
                APIBridge.callAPI('xyc_bcaf_create', [$scope.scanning_detail, $scope.l_data]).then(function (result) {
                    if (result) {
                        $ionicLoading.hide();
                        xyc_fil_01_requisition.init();
                        $state.go(page);
            ...
            $scope.bcmeCreate = function(parameter) {
                ...
                $ionicLoading.show();
                APIBridge.callAPI('xyc_bcme_create', [parameter, $scope.l_data]).then(function (result) {
                    $ionicLoading.hide();
                    if (result) {
            ...
            var bcmeBcaeBcafGet = function(isShowUpcoming, type_no) {
                ...
                $ionicLoading.show();
                APIBridge.callAPI('xyc_bcme_ae_af_get', [obj, $scope.l_data]).then(function (result) {
                    $ionicLoading.hide();
