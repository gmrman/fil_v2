define(["API", "APIS", 'AppLang', 'views/app/xyc_fil_01/requisition.js', 'array', 'Directives', 'ReqTestData', 'ionic-popup',
    ...
    return ['$scope', '$state', '$stateParams', '$timeout', '$filter', 'AppLang', 'APIService', 'APIBridge',
        ...
        function($scope, $state, $stateParams, $timeout, $filter, AppLang, APIService, APIBridge,
            ...
            $scope.scan = function(type) {
                ...
                APIBridge.callAPI('scanBarcode', [{}]).then(function(result) {
                    if (result) {
                        ...
                        $timeout(function() {
                            if (type == '2') {
                                $scope.scanSMark(result.data[0].barcode.trim());
                            } else {
            ...
            $scope.scanSMark = function (scanning) {
                $scope.setsMark(scanning);
            ...
            $scope.checkScan = function(scanning) {
                ...
                if (commonService.isNull($scope.scaninfo.shipping_mark)) {
                    userInfoService.getVoice($scope.xlangs.shipping_mark_error1, function () {
                        $scope.setFocusMe(false);
                        $scope.setFocusSmark(true);
                    });
                    return;
                } else {
                    var shipping_mark = $scope.scaninfo.shipping_mark;
                    var index = $scope.scanning_detail.findIndex(function (t) {
                        return commonService.isEquality(t.shipping_mark, shipping_mark);
                    });
                    if (index > -1) {
                        //已扫描
                ...
                if ($scope.l_data.has_source) {
                    ...
                    for (var i = 0; i < barcodedata_length; i++) {
                        ...
                        for (var j = 0; j < sourceDocDetail.length; j++) {
                            ...
                            if (commonService.isEquality(value.shipping_mark_no, $scope.scaninfo.shipping_mark_no) &&
                                commonService.isEquality(value.item_no, element.item_no) &&
            ...
            var lot_control_type_1 = function(barcode_info) {
                ...
                if ($scope.l_data.has_source) {
                    ...
                    console.log(xyc_fil_01_requisition.computed_doc_detail);
                    //檢查單據各項次是否有缺少批號
                    for (var i = 0; i < xyc_fil_01_requisition.computed_doc_detail.length; i++) {
                        var element = xyc_fil_01_requisition.computed_doc_detail[i];
            ...
            var lot_control_type_3 = function(barcode_info) {
                ...
                if ($scope.l_data.has_source) {
                    console.log(xyc_fil_01_requisition.computed_doc_detail);
                    angular.forEach(xyc_fil_01_requisition.computed_doc_detail, function(value) {
            ...
            var setTempGood = function(barcode_info) {
                ...
                var shipping_mark = $scope.scaninfo.shipping_mark;
                var shipping_mark_no = shipping_mark.substr(0, shipping_mark.length - 4);
                var temp = {
                    shipping_mark_no: shipping_mark_no,
 ...
 if (!(commonService.isEquality(sourceDocDetail[i].shipping_mark_no, $scope.scaninfo.shipping_mark_no)) ||
                   
                    (sourceDocDetail[i].allow_doc_qty === 0 ||
                    ...
                    if ($scope.scanning_detail.length > 0) {
                        ...
                        angular.forEach($scope.scanning_detail, function(value) {
                            if (commonService.isEquality(value.shipping_mark_no, sourceDocDetail[i].shipping_mark_no) &&
                                commonService.isEquality(value.item_no, sourceDocDetail[i].item_no) &&
                    ...
                    //單位轉換
                    item.qty = xyc_fil_01_requisition.accConversion(item.qty, sourceDocDetail[i]);
                    item.barcode_qty = xyc_fil_01_requisition.accConversion(item.barcode_qty, sourceDocDetail[i]);
                    item.maxqty = xyc_fil_01_requisition.accConversion(item.maxqty, sourceDocDetail[i]);
                    item.barcode_inventory_qty = xyc_fil_01_requisition.accConversion(item.barcode_inventory_qty, sourceDocDetail[i]);
                    item.used_barcode_inventory_qty = xyc_fil_01_requisition.accConversion(item.barcode_inventory_qty, sourceDocDetail[i]);
            ...
            var distributionQty = function(tempArray, qty, all_doc_qty) {
                ...
                for (var i = 0; i < tempArray.length; i++) {
                    ...
                    if (isDistributionQty) {
                        ...
                        $scope.scaninfo.shipping_mark = "";
                        $scope.scaninfo.shipping_mark_no = "";
