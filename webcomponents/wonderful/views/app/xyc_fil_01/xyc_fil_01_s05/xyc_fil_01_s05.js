define(["API", "APIS", 'AppLang', 'views/app/xyc_fil_01/requisition.js', 'array', 'Directives', 'ReqTestData', 'ionic-popup',
    ...
    return ['$scope', '$state', '$stateParams', '$timeout', '$filter', 'AppLang', 'APIService', 'APIBridge',
        ...
        function($scope, $state, $stateParams, $timeout, $filter, AppLang, APIService, APIBridge,
            ...
            $scope.collection_item_height = rows * 40 - ((rows - 1) * 5);
            ...
            var bcmeGet = function() {
                APIBridge.callAPI('xyc_bcme_get', [$scope.l_data]).then(function(result) {
                    $ionicLoading.hide();
                    if (result) {
                        $timeout(function() {
            ...
            var bcafGet = function() {
                APIBridge.callAPI('xyc_bcaf_get', [$scope.l_data]).then(function(result) {
                    $ionicLoading.hide();
                    if (result) {
                        $timeout(function() {
                            var submit_show = result.data[0].submit_show;
            ...
            var getNewData = function (scan) {
                var newScan = [];
                scan.forEach(function (t) {
                    var tmp = angular.copy(t);
                    tmp.scan_detail = [];
                    var detail = t.scan_detail;
                    var groupBySMark = [];
                    detail.forEach(function (t2) {
                        var tmp_smark = {
                            xshipping_mark_no: t2.xshipping_mark_no
                        };
                        var index = groupBySMark.findIndex(function (t3) {
                            return commonService.isEquality(t3.xshipping_mark_no, tmp_smark.xshipping_mark_no);
                        });
                        if (index == -1) {
                            groupBySMark.push(tmp_smark);
                        }
                    });

                    groupBySMark.forEach(function (t2) {
                        var newDetail = [];
                        var xshipping_mark_no = t2.xshipping_mark_no;
                        detail.forEach(function (t3) {
                            if(commonService.isEquality(
                                    t3.xshipping_mark_no, xshipping_mark_no)){
                                newDetail.push(t3);
                            }
                        });
                        tmp.scan_detail = angular.copy(newDetail);
                        newScan.push(angular.copy(tmp));
                    });
                })
                return newScan;
            }
            ...
            var bcmeBcaeBcafDelete = function(doc_no, print_param) {
                ...
                if (($scope.page_params.program_job_no == "3" && $scope.page_params.scan_type == "1") ||
                    ...
                    page = "xyc_fil_01_s02.xyc_fil_01_s07";
                }
                if (($scope.page_params.program_job_no == "1" && $scope.page_params.scan_type == "3") ||
                    ...
                    page = "xyc_fil_01_s02.xyc_fil_01_s03";
                }
            ...
            var bcaeBcafUploadCreate = function() {
                $ionicLoading.show();
                APIBridge.callAPI('xyc_bcae_bcaf_upload_create', [$scope.l_data]).then(function(result) {
                    $ionicLoading.hide();
                    if (result) {
            ...
            var app_bcscanwsupload_create = function(scan, packing_detail) {
                var webTran = {
                    ...
                    service: 'app.xInsertSalesIssue.create',
                    parameter: {
            ...
            $scope.docSlipShow = function() {
                ...
                $ionicModal.fromTemplateUrl('views/app/xyc_fil_01/xyc_fil_01_s05/xyc_fil_01_s05_01.html', {
                    scope: $scope
                }).then(function(modal) {
