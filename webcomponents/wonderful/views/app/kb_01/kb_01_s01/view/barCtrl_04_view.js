app.controller("barCtrl_04", ["$scope", "$timeout", "AppLang", function ($scope, $timeout, AppLang) {
   $scope.langs = AppLang.langs
    
   $scope.$on("changeData", function (evt, dataList, showme) {
      if (showme[3]) {
         //無法取得資料時，不顯示圖表
         var data = dataList["barCtrl_04"].data
         var labels = $scope.getDateLabel("year", dataList["barCtrl_04"].label)
