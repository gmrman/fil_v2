app.controller("barCtrl_02", ["$scope", "$timeout", "AppLang", function ($scope, $timeout, AppLang) {
   $scope.langs = AppLang.langs

   $scope.$on("changeData", function (evt, dataList, showme) {
      if (showme[1]) {
         //無法取得資料時，不顯示圖表
         var data = dataList["barCtrl_02"].data
         var labels = $scope.getDateLabel("year", dataList["barCtrl_02"].label)
