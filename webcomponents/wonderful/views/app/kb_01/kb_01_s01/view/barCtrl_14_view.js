app.controller("barCtrl_14", ["$scope", "$timeout", "AppLang", function ($scope, $timeout, AppLang) {
   $scope.langs = AppLang.langs

   $scope.$on("changeData", function (evt, dataList, showme) {
      if (showme[13]) {
         //無法取得資料時，不顯示圖表
         var data = dataList["barCtrl_14"].data
         var labels = $scope.getDateLabel("last", dataList["barCtrl_14"].label)
