app.controller("barCtrl_12", ["$scope", "$timeout", "AppLang", function ($scope, $timeout, AppLang) {
   $scope.langs = AppLang.langs

   $scope.$on("changeData", function (evt, dataList, showme) {
      if (showme[11]) {
         //無法取得資料時，不顯示圖表
         var data = dataList["barCtrl_12"].data
         var labels = $scope.getDateLabel("last", dataList["barCtrl_12"].label)
