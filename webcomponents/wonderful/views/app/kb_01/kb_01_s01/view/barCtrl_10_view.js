app.controller("barCtrl_10", ["$scope", "$timeout", "AppLang", function ($scope, $timeout, AppLang) {
   $scope.langs = AppLang.langs

   $scope.$on("changeData", function (evt, dataList, showme) {
      if (showme[9]) {
         //無法取得資料時，不顯示圖表
         var data = dataList["barCtrl_10"].data
         var labels = $scope.getDateLabel("last", dataList["barCtrl_10"].label)
