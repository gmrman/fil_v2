app.controller("barCtrl_06", ["$scope", "$timeout", "AppLang", function ($scope, $timeout, AppLang) {
   $scope.langs = AppLang.langs

   $scope.$on("changeData", function (evt, dataList, showme) {
      if (showme[5]) {
         //無法取得資料時，不顯示圖表
         var data = dataList["barCtrl_06"].data
         var labels = $scope.getDateLabel("year", dataList["barCtrl_06"].label)
