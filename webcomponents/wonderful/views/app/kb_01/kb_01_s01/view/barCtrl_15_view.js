app.controller("barCtrl_15", ["$scope", "$timeout", "AppLang", function ($scope, $timeout, AppLang) {
   $scope.langs = AppLang.langs

   $scope.$on("changeData", function (evt, dataList, showme) {
      if (showme[14]) {
         //無法取得資料時，不顯示圖表
         var data = dataList["barCtrl_15"].data
         var labels = $scope.getDateLabel("month", dataList["barCtrl_15"].label)
