app.controller("barCtrl_09", ["$scope", "$timeout", "AppLang", function ($scope, $timeout, AppLang) {
   $scope.langs = AppLang.langs

   $scope.$on("changeData", function (evt, dataList, showme) {
      if (showme[8]) {
         //無法取得資料時，不顯示圖表
         var data = dataList["barCtrl_09"].data
         var labels = $scope.getDateLabel("month", dataList["barCtrl_09"].label)
