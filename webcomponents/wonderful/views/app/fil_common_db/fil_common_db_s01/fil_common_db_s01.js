    ...
    return ['$scope', '$state', '$timeout', '$filter', 'AppLang', 'APIService', 'APIBridge',
        ...
        function($scope, $state, $timeout, $filter, AppLang, APIService, APIBridge,
            ...
            $scope.showQbeTodoNotice = function() {
                $ionicLoading.show();
                $ionicModal.fromTemplateUrl('views/app/fil_common_db/fil_common_db_s01/fil_common_db_s01_01.html', {
                    scope: $scope
                }).then(function(modal) {
            ...
            var app_todo_doc_get = function() {
                ...
                APIService.Web_Post(webTran, function(res) {
                    ...
                    console.log(res);
                    var parameter = res.payload.std_data.parameter;
                    fil_common_db_requisition.setDocarry($scope.doc_array);
            ...
            if (!commonService.isNull(fil_common_db_requisition.scanning)) {
                var scanning = angular.copy(fil_common_db_requisition.scanning);
                fil_common_db_requisition.scanning = "";
                $scope.getInfo({
                    "doc_no": scanning,
                    "seq": "",
                });
