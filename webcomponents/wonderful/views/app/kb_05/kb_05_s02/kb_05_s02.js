define(["API", "APIS", "ionic-popup", "AppLang", "Directives", "ReqTestData", "commonService", "views/app/kb_05/kb_05_s01/js/PopupService.js"], function () {
   return ["userInfoService", "$state", "$stateParams", "$scope", "kb_05_PopupService", "$ionicLoading", "$ionicPopup", "IonicClosePopupService", "IonicPopupService", "APIService", "APIBridge", "$timeout", "AppLang", "ReqTestData", "commonService", "$q", "$ionicScrollDelegate",
      function (userInfoService, $state, $stateParams, $scope, kb_05_PopupService, $ionicLoading, $ionicPopup, IonicClosePopupService, IonicPopupService, APIService, APIBridge, $timeout, AppLang, ReqTestData, commonService, $q, $ionicScrollDelegate) {
         ...
         function page_init() {
            ...
            $scope.deal_with_out = [
               {
                  "type": "A01",
                  "name": $scope.langs.work_order_in,
                  "value": false
               },
               {
                  "type": "A02",
                  "name": $scope.langs.hybrid_in,
                  "value": false
               },
               {
                  "type": "A03",
                  "name": $scope.langs.sell_back_in,
                  "value": false
               },
               {
                  "type": "A04",
                  "name": $scope.langs.return_material + $scope.langs.doc,
                  "value": false
               }
            ]
         ...
         function getSingle() {
            ...
            var webTran = {
               "service": "app.km.oh.stkin.doclist.get",
               "parameter": {
         ...
         $scope.warehouseShow = function () {
            kb_05_PopupService.showWarehouseModal(angular.copy(userInfoService.warehouse), $scope.warehouseList, $scope.setWarehouse, function () { })
         }
         ...
         $scope.singleShow = function () {
            if (chkSource()) {
               if (ReqTestData.testData) {
                  //取得單別資料
                  getSingle()
                  kb_05_PopupService.showSingleModal(angular.copy($scope.sel_Single), $scope.singleList, $scope.setSingle, function () { })
               }
               else {
         ...
         function setShowSetting() {
            ...
            if (commonService.Platform == "Chrome") {
               ...
               APIBridge.callAPI("kb_05_upd_setting", [parameter]).then(function (result) {
                  $ionicLoading.hide()
                  $state.go("kb_05_s01")
               }, function (error) {
                  $ionicLoading.hide()
                  IonicPopupService.errorAlert("kb_05_upd_setting fail")
                  console.log(error)
               })
            }
         }
