define(["app"], function(app) {
    app.service('AppLang', [function() {
        ...
        self.langs_reference = {
            ...
            "qc_maxRefQty_error": ["檢驗参考數量不可大於待驗批量！", "检验参考数量不可大于待验批量！", "qc_maxRefQty_error"],
            ...
            //加值版
            "kb_01_s01": ["收貨統計看板", "收货统计看板", "receiveing_statistical_kanban"],
            "kb_02_s01": ["進貨導引", "进货导引", "purchase_guide"],
            "kb_02_s02": ["作業參數設定", "作业参数设定", "job_parameter_setting"],
            "kb_03_s01": ["待出導引", "待出导引", "wait_delivery"],
            "kb_03_s03": ["出倉維護", "出仓维护", "delivery_maintain"],
            "kb_05_s01": ["待入導引", "待入导引", "wait_put_in_storage"],
            "kb_05_s03": ["入倉維護", "入仓维护", "put_in_storage_maintain"],
            "job_setting": ["作業設定", "作业设定", "job_setting"],
            "page_back": ["返回", "返回", "return"],
            "actual_storage_number": ["實際入庫數", "实际入库数", "actual_storage_number"],
            "work_order_in": ["工入", "工入", "work_rder_storage"],
            "hybrid_in": ["雜入", "杂入", "sundry_storage"],
            "sell_back_in": ["銷退單", "销退单", "sale_return_single"],
            "first": ["第", "第", "No."],
            "completion_date": ["預計完工日", "预计完工日", "estimated_complete_date"],
            "start_date": ["預計開工日", "预计开工日", "estimated_start_date"],
            "customer_name": ["客戶名稱", "客户名称", "customer_name"],
            "shipments_qty": ["已出貨量", "已出货量", "already_shipments_qty"],
            "agreed_shipping_date": ["預計出貨日", "预计出货日", "estimated_shipments_date"],
            "shipping_date": ["預計交貨日", "预计交货日", "estimated_delivery_date"],
            "arrival_date": ["預計到廠日", "预计到厂日", "estimated_arrival_date"],
            "stock_in_date": ["預計入庫日", "预计入库日", "estimated_storage_day"],
            "qcrej": ["驗退", "验退", "qc_return"],
            "qc_qty": ["待驗量", "待验量", "to_be_qc_qty"],
            "supplier_name": ["供應商名稱", "供应商名称", "supplier_name"],
            "vendor_name": ["簡稱", "简称", "short_name"],
            "vendor_fname": ["全名", "全名", "full_name"],
            "address": ["地址", "地址", "address"],
            "phonenumber": ["電話", "电话", "phone_number"],
            "faxnumber": ["傳真", "传真", "fax_number"],
            "warehouse_picked": ["出倉揀料", "出仓拣料", "delivery_pick_materials"],
            "warehouse_sum": ["出倉匯總", "出仓汇总", "delivery_collection"],
            "warehouse_picked_in": ["入倉揀料", "入仓拣料", "put_in_storage_pick_materials"],
            "warehouse_sum_in": ["入倉匯總", "入仓汇总", "put_in_storage_collection"],
            "should_picking": ["應揀量", "应拣量", "should_pick_qty"],
            "picked": ["已揀量", "已拣量", "picked_qty"],
            "picking": ["揀料量", "拣料量", "picking_qty"],
            "doc_number": ["單數", "单数", "doc_number"],
            "doc_seq": ["單項", "单项", "doc_seq"],
            "warehouse_back": ["倉退", "仓退", "warehouse_return"],
            "guide_the_role": ["導引角色", "导引角色", "guide_the_role"],
            "page_number": ["每頁筆數", "每页笔数", "count_of_per_page"],
            "customize": ["自定義", "自定义", "customize"],
            "single": ["單別", "单别", "single"],
            "picking_material": ["領料", "领料", "picking_material"],
            "miscellaneous": ["雜發", "杂发", "miscellaneous_materials"],
            "day_number": ["日數", "日数", "day_number"],
            "detail_cnt": ["項數", "项数", "detail_count"],
            "compared": ["對比", "对比", "compared"],
            "item_status": ["料況", "料况", "item_status"],
            "deal_with_out": ["待出處理", "待出处理", "delivery_deal_with"],
            "deal_with_in": ["待入處理", "待入处理", "put_in_storage_deal_with"],
            "refresh": ["下拉刷新", "下拉刷新", "drop_down_refresh"],
            "quantity": ["筆", "笔", "count"],
            "count": ["數", "数", "count"],
            "day": ["天", "天", "day"],
            "list": ["單", "单", "single"],
            "stage": ["階段", "阶段", "stage"],
            "limited": ["限制", "限制", "limited"],
            "mode": ["方式", "方式", "mode"],
            "sorting": ["排序", "排序", "sorting"],
            "prepaid_date": ["預交日", "预交日", "estimated_delivery_date"],
            "image": ["圖片", "图片", "image"],
            "view": ["檢視", "检视", "view"],
            "package": ["包裝", "包装", "package"],
            "order_no": ["訂單", "订单", "order_no"],
            "check_accept": ["點收", "点收", "check_accept"],
            "more_features": ["功能選擇", "功能选择", "more_features"],
            "manufacturers": ["廠商", "厂商", "manufacturers"],
            "manufacturers_name": ["廠商名稱", "厂商名称", "manufacturers_name"],
            "integrated": ["綜合", "综合", "integrated"],
            "statistical": ["統計", "统计", "statistical"],
            "kanban": ["看板", "看板", "kanban"],
            "moneyTable": ["金額表", "金额表", "amount_table"],
            "notSelItem": ["未搜尋到此筆資料<br />{0}", "未搜寻到此笔资料<br />{0}", "Did not find this information.<br />{0}"],

            "error": {
               "MD001": ["請至少勾選一個顯示項目！", "请至少勾选一个显示项目！", "Please check at least one display item!"],
               "MD002": ["請先設置單據類型!", "请先设置单据类型!", "Please set the document type first!"],
               "MD003": ["請先設置倉庫資料!", "请先设置仓库资料!", "Please set the warehouse information first!"],
               "MD004": ["請先設置資料來源!", "请先设置资料来源!", "Please set the data source first!"],
               "MD005": ["數量不可超過原單據數量!", "数量不可超过原单据数量!", "The number can not exceed the original number of documents!"],
               "MD006": ["請至少勾選一筆資料!", "请至少勾选一笔资料!", "Please check at least one item!"],
               "MD007": ["揀料數量匯總 {0} 超過應揀量 {1} !", "拣料数量汇总 {0} 超过应拣量 {1} !", "The total number of picking materials {0} Exceeds the amount of pick {1} !"],
               "MD008": ["數量已達最小值!", "数值已达最小值!", "The value has reached the minimum value!"],
               "MD009": ["揀料量 {0} 不可大於應揀量 {1} !", "拣料量 {0} 不可大于应拣量 {1} !", "Pick the amount of material {0}, not be greater than the amount of pick {1} !"],
               "MD010": ["數量不可大於庫存數量!", "数量不可大于库存数量!", "The quantity can not be greater than the stock quantity!"],
               "MD011": ["資料來源並非{0}!", "资料来源并非{0}!", "Source is not {0}!"],
               "MD012": ["不良數量已經符合報廢數量!", "不良数量已经符合报废数量!", "The number of bad has been met the scrap number!"],
               "MD013": ["不可超過報廢數量", "不可超过报废数量", "Can not exceed the number of scrapped"],
               "MD014": ["請選擇不良原因!", "请选择不良原因!", "Please select the reason for the bad!"],
               "MD015": ["請先設置單據類型或倉庫!", "请先设置单据类型或仓库!", "Please set the document type or warehouse first!"],
               "MD016": ["已達此批號可揀量!", "已达此批号可拣量!", "Has reached this batch pickable amount!"],
               "MD017": ["此批號剩餘可揀量不足", "此批号剩余可拣量不足", "This lot is not enough to pick up"],
               "MD018": ["已顯示全批號資料", "已显示全批号资料", "All batch information has been displayed"],
               "MD019": ["單據剩餘可揀料量為零!", "单据剩余可拣料量为零!", "The amount of remaining rejects on the document is zero!"],
               "MD020": ["欄位{0}不可空白!", "栏位{0}不可空白!", "Field {0} Can not be blank!"],
               "MD021": ["此批號已存在清單中!", "此批号已存在清单中!", "Lot number already exists in the list!"],
               "MD022": ["數量不可為零!", "数量不可为零!", "Number can not be zero!"],
               "MD023": ["數量 {0} 不可小於批號已分配的數量 {1}!", "数量 {0} 不可小于批号已分配的数量 {1} !", "Quantity: {0}, not less than the quantity of lots assigned: {1} !"],
               "MD024": ["WF產品不提供此檢視功能", "WF产品不提供此检视功能", "WF products do not provide view capability."],
               "MD025": ["WF產品不支援標籤列印", "WF产品不支援标签列印", "WF products do not support label printing."],
            },

            "year": ["年度", "年度", "year"],
            "periodic": ["期別", "期别", "periodic"],
            "month": ["月份", "月份", "month"],
            "last": ["期間", "期间", "last"],
            "barCtrl_13": ["月份未完成處理表", "月份未完成处理表", "month_unfinished_table"],
            "barCtrl_14": ["期間未完成處理表", "期间未完成处理表", "last_unfinished_table"],
            "barCtrl_15": ["月份逾期未交表", "月份逾期未交表", "month_overdue_table"],
            "barCtrl_16": ["期間逾期未交表", "期间逾期未交表", "last_overdue_table"],
            "estimated_amount": ["預測金額", "预测金额", "estimated_amount"],
            "actual_amount": ["實際金額", "实际金额", "actual_amount"],
            "predict_project": ["預測項目", "预测项目", "predict_project"],
            "actual_project": ["實際項目", "实际项目", "actual_project"],
        };
        ...
        function setlang(index) {
            ...
            for (var i = 0; i < array.length; i++) {
                ...
                if (angular.isArray(self.langs_reference[temp])) {
                   ...
                   //ZHEN-來源為Object時在往下找資料
                   if (angular.isObject(self.langs_reference[temp])) {
                      ref = {}
                      var temp_array = Object.keys(self.langs_reference[temp])
                      for (var j = 0; j < temp_array.length; j++) {
                         var temp_field = temp_array[j]
                         ref[temp_field] = self.langs_reference[temp][temp_field][index]
                      }
                   }
                }
                self.langs[temp] = ref;
