...
TYPE scandetail                           RECORD
      enterprise_no                          LIKE app_base_bcaf_t.bcafent, #企业编号,
      site_no                                LIKE app_base_bcaf_t.bcafsite,#门店编号,
      info_id                                LIKE app_base_bcaf_t.bcaf001, #信息ID,
      info_lot_no                            LIKE app_base_bcaf_t.bcaf002, #信息批号,
      scan_doc_no                            LIKE app_base_bcaf_t.bcaf003, #扫描单号,
      scan_seq                               LIKE app_base_bcaf_t.bcaf004, #扫描项次,
      in_out_no                              LIKE app_base_bcaf_t.bcaf005, #出入库码,
      barcode_no                             STRING,                       #条码编号,
      item_no                                LIKE app_base_bcaf_t.bcaf007, #料件编号,
      item_feature_no                        LIKE app_base_bcaf_t.bcaf008, #产品特征,
      item_feature_name                      LIKE app_base_bcaf_t.bcaf009, #产品特征说明,
      warehouse_no                           LIKE app_base_bcaf_t.bcaf010, #库位,
      storage_spaces_no                      LIKE app_base_bcaf_t.bcaf011, #储位,
      lot_no                                 STRING,                       #批号,
      inventory_management_features          LIKE app_base_bcaf_t.bcaf013, #库存管理特征,
      plot_no                                LIKE app_base_bcaf_t.bcaf014, #制造批号,
      serial_no                              LIKE app_base_bcaf_t.bcaf015, #制造序号,
      picking_qty                            LIKE app_base_bcaf_t.bcaf016, #拣货数量,
      picking_unit_no                        LIKE app_base_bcaf_t.bcaf017, #拣货单位,
      picking_date                           LIKE app_base_bcaf_t.bcaf018, #拣货日期时间,
      doc_no                                 LIKE app_base_bcaf_t.bcaf020, #单号,
      seq                                    LIKE app_base_bcaf_t.bcaf021, #项次,
      line_seq                               LIKE app_base_bcaf_t.bcaf022, #项序,
      batch_seq                              LIKE app_base_bcaf_t.bcaf023, #分批序,
      source_unit_no                         LIKE app_base_bcaf_t.bcaf031, #来源单位,
      source_qty                             LIKE app_base_bcaf_t.bcaf032, #来源数量,
      objective_unit_no                      LIKE app_base_bcaf_t.bcaf033, #目的单位,
      objective_qty                          LIKE app_base_bcaf_t.bcaf034, #目的数量,
      conversion_qty                         LIKE app_base_bcaf_t.bcaf035, #换算后数量
      ...
      reference_unit_no                      LIKE app_base_bcaf_t.bcaf044, #參考單位
      reference_qty                          LIKE app_base_bcaf_t.bcaf045  #參考數量
                                          END RECORD
...
PUBLIC FUNCTION fil3_bcae_bcaf_upload_create(p_jsonarr)
   ...
   LET l_sql = " SELECT bcme023, bcme024, bcme025, bcme026        ",   
   ...
   PREPARE bcme_doc_pre FROM l_sql
   ...
   IF l_success THEN
      ...
      FOREACH s_scan_cur INTO l_bcae_t.bcaeent, l_bcae_t.bcaesite, l_bcae_t.bcae001, l_bcae_t.bcae002, l_bcae_t.bcae003, 
         ...
         FOREACH s_scandetail_cur USING head[l_i].enterprise_no, head[l_i].site_no , head[l_i].info_id,
            ...
            IF l_data.bcae006 = "1-1A" OR l_data.bcae006 = "1-2A" OR l_data.bcae006 = "3-1A" OR l_data.bcae006 = "7-4A" THEN
               EXECUTE bcme_doc_pre USING l_data.bcae006,
                                          head[l_i].scan_detail[l_j].doc_no,
                                          head[l_i].scan_detail[l_j].seq,
                                          head[l_i].scan_detail[l_j].line_seq,
                                          head[l_i].scan_detail[l_j].batch_seq
                                     INTO l_bcme.doc_no, l_bcme.seq, l_bcme.line_seq, l_bcme.batch_seq
            
               LET head[l_i].scan_detail[l_j].doc_no    = l_bcme.doc_no
               LET head[l_i].scan_detail[l_j].seq       = l_bcme.seq
               LET head[l_i].scan_detail[l_j].line_seq  = l_bcme.line_seq
               LET head[l_i].scan_detail[l_j].batch_seq = l_bcme.batch_seq
            END IF 
