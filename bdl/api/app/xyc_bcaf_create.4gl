...
SCHEMA ds
 ...
 TYPE type_scan     RECORD   #單身記錄
         barcode_no                        LIKE xyc_app_base_bcaf_t.bcaf006,  #條碼
         item_no                           LIKE xyc_app_base_bcaf_t.bcaf007,  #料號
         ...
         item_spec                         LIKE xyc_app_base_bcaf_t.bcafspec, #規格
         multi_unit_type                   LIKE xyc_app_base_bcaf_t.bcafMunit,#多單位的單位控管方式
         warehouse_no                      LIKE xyc_app_base_bcaf_t.bcaf010,  #倉庫
         storage_spaces_no                 LIKE xyc_app_base_bcaf_t.bcaf011,  #儲位
         lot_no                            LIKE xyc_app_base_bcaf_t.bcaf012,  #批號
         unit                              LIKE xyc_app_base_bcaf_t.bcaf017,  #單位
         qty                               LIKE xyc_app_base_bcaf_t.bcaf016,  #數量
         ...
         outgoing_warehouse_no             LIKE xyc_app_base_bcaf_t.bcaf036,  #撥出倉庫
         outgoing_storage_spaces_no        LIKE xyc_app_base_bcaf_t.bcaf037,  #撥出儲位
         ...
         source_no                         LIKE xyc_app_base_bcaf_t.bcaf020,  #单号
         seq                               LIKE xyc_app_base_bcaf_t.bcaf021,  #项次
         doc_line_seq                      LIKE xyc_app_base_bcaf_t.bcaf022,  #项序
         doc_batch_seq                     LIKE xyc_app_base_bcaf_t.bcaf023,  #分批序
         ...
         shipping_mark_no                  LIKE xyc_app_base_bcaf_t.xshipping_mark_no,
         shipping_mark                     LIKE xyc_app_base_bcaf_t.xshipping_mark_cn
...
#掃描紀錄檔單頭預設值
FUNCTION xyc_gat_bcae_def(p_scan,p_data)
...
FUNCTION xyc_gat_bcaf_def(p_bcae_t,p_scan,p_data)
   DEFINE p_bcae_t          RECORD LIKE app_base_bcae_t.*
   DEFINE p_scan            type_scan
   DEFINE l_bcaf_t          RECORD LIKE xyc_app_base_bcaf_t.*
   DEFINE l_bcaf004         LIKE xyc_app_base_bcaf_t.bcaf004      #掃描項次
   DEFINE p_data            type_bcae
   ...
   LET l_bcaf_t.xshipping_mark_no  = p_scan.shipping_mark_no
   LET l_bcaf_t.xshipping_mark_cn  = p_scan.shipping_mark

   IF  cl_null(l_bcaf_t.xshipping_mark_no)  THEN 
    LET l_bcaf_t.xshipping_mark_no  = ' '
    LET l_bcaf_t.xshipping_mark_cn  = ' '
   END IF 
...
#新增掃描紀錄
FUNCTION xyc_bcaf_create(p_jsonarr)
   ...
   IF l_cnt > 0 THEN
      ...
      DELETE FROM xyc_app_base_bcaf_t 
       WHERE bcafent = g_userInfo.enterprise_no
         AND bcafsite = g_userInfo.site_no
   ...
   FOR l_i = 1 TO l_scan.getLength()
      ...
      IF l_cnt = 0 THEN
         CALL xyc_gat_bcae_def(l_scan[l_i].*,l_data.*) RETURNING l_bcae_t.*
      ...
      CALL xyc_gat_bcaf_def(l_bcae_t.*,l_scan[l_i].*,l_data.*) RETURNING l_bcaf_t.*
      
      INSERT INTO xyc_app_base_bcaf_t VALUES(l_bcaf_t.*)
      IF SQLCA.SQLCODE THEN
         LET g_status.code = FALSE
         LET g_status.message = SQLCA.SQLCODE
      END IF
