...
TYPE detail             RECORD
      item_no              LIKE app_base_bcaf_t.bcaf007,   #料件編號
      item_feature_no      LIKE app_base_bcaf_t.bcaf008,   #產品特徵
      item_feature_name    LIKE app_base_bcaf_t.bcaf009,   #產品特徵說明
      item_name	         LIKE app_base_bcaf_t.bcafname,   #品名
      item_spec            LIKE app_base_bcaf_t.bcafname,   #規格
      already_qty          LIKE app_base_bcaf_t.bcaf016,   #揀貨數量
      unit                 LIKE app_base_bcaf_t.bcaf017,   #揀貨單位
      multi_unit_type      LIKE app_base_bcaf_t.bcafMunit, #多單位的單位控管方式
      reference_unit_no    LIKE app_base_bcaf_t.bcaf042,   #參考單位
      already_ref_qty      LIKE app_base_bcaf_t.bcaf043,   #參考數量
      valuation_unit_no    LIKE app_base_bcaf_t.bcaf044,   #計價單位
      already_val_qty      LIKE app_base_bcaf_t.bcaf045,   #計價數量
      inventory_unit       LIKE app_base_bcaf_t.bcaf046,   #庫存單位
      already_inv_qty      LIKE app_base_bcaf_t.bcaf047,   #庫存數量
      packing_barcode      LIKE app_base_bcaf_t.bcafPcode, #裝箱條碼
      packing_qty          LIKE app_base_bcaf_t.bcafPqty,  #裝箱數量
      submit_show          BOOLEAN
                        END RECORD
PUBLIC FUNCTION bcaf_get(p_json)
   ...
   DEFINE l_bcaf           RECORD
            bcaf007           LIKE app_base_bcaf_t.bcaf007,   #料件編號
            bcaf008           LIKE app_base_bcaf_t.bcaf008,   #產品特徵
            bcaf009           LIKE app_base_bcaf_t.bcaf009,   #產品特徵說明
            bcafname          LIKE app_base_bcaf_t.bcafname,  #品名
            bcafspec          LIKE app_base_bcaf_t.bcafspec,  #規格
            bcaf016           LIKE app_base_bcaf_t.bcaf016,   #數量
            bcaf017           LIKE app_base_bcaf_t.bcaf017,   #揀貨單位
            bcafMunit         LIKE app_base_bcaf_t.bcafMunit, #多單位的單位控管方式
            bcaf042           LIKE app_base_bcaf_t.bcaf042,   #參考單位
            bcaf043           LIKE app_base_bcaf_t.bcaf043,   #參考數量
            bcaf044           LIKE app_base_bcaf_t.bcaf044,   #計價單位
            bcaf045           LIKE app_base_bcaf_t.bcaf045,   #計價數量
            bcaf046           LIKE app_base_bcaf_t.bcaf046,   #庫存單位
            bcaf047           LIKE app_base_bcaf_t.bcaf047,   #庫存數量
            bcafPcode         LIKE app_base_bcaf_t.bcafPcode, #裝箱條碼
            bcafPqty          LIKE app_base_bcaf_t.bcafPqty   #裝箱數量
                           END RECORD
