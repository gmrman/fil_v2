...
TYPE type_capp_kb_03_setting_t   RECORD
   ent           LIKE capp_kb_03_setting_t.ent,        #企業編號
   site          LIKE capp_kb_03_setting_t.site,       #營業據點
   source        LIKE capp_kb_03_setting_t.source,     #來源設定 0.單據類型  1.倉庫
   A01           LIKE capp_kb_03_setting_t.A01,        #出貨
   A02           LIKE capp_kb_03_setting_t.A02,        #領料
   A03           LIKE capp_kb_03_setting_t.A03,        #雜發
   A04           LIKE capp_kb_03_setting_t.A04         #倉退
               END RECORD 

TYPE type_capp_kb_03_warehouse_t   RECORD
   warehouse      LIKE capp_kb_03_warehouse_t.warehouse  #倉庫
               END RECORD 

TYPE type_capp_kb_03_single_t   RECORD
   single         LIKE capp_kb_03_single_t.single     #單別
               END RECORD 
               
PUBLIC FUNCTION kb_03_upd_setting(p_json)
   DEFINE p_json     util.JSONArray
   DEFINE tempobj    util.JSONObject
   DEFINE l_master   RECORD
             setting      type_capp_kb_03_setting_t,
             warehouse    DYNAMIC ARRAY OF type_capp_kb_03_warehouse_t,
             single       DYNAMIC ARRAY OF type_capp_kb_03_single_t
                 END RECORD
   DEFINE l_i        INTEGER
   ...
   DELETE FROM capp_kb_03_setting_t
    WHERE ent = l_master.setting.ent
      AND site = l_master.setting.site

   DELETE FROM capp_kb_03_warehouse_t
    WHERE ent = l_master.setting.ent
      AND site = l_master.setting.site

   DELETE FROM capp_kb_03_single_t
    WHERE ent = l_master.setting.ent
      AND site = l_master.setting.site

   INSERT INTO capp_kb_03_setting_t(ent, site, source, A01, A02, A03, A04)
   ...
   FOR l_i = 1 TO l_master.warehouse.getLength()  
      ...
      INSERT INTO capp_kb_03_warehouse_t(ent, site, warehouse)
   ...
   FOR l_i = 1 TO l_master.single.getLength()  
      ...
      INSERT INTO capp_kb_03_single_t(ent, site, single)
      VALUES (l_master.setting.ent,l_master.setting.site,
